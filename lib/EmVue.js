...
class EmVue {
	...
	async getEmpDayUsage(outputunits=0) {
		let usage = {};
		let [status, data] = await this.getPromiseDeviceListUsage({instant:moment().utc().subtract(1, "days").endOf("day").format(),scale:Scale.DAY});

		if (status === 401) {           // token expired
			[status, data] = await this.getNewTokens();
			this.storeTokens(data.AuthenticationResult);
			[status, data] = await this.getPromiseDeviceListUsage({instant:moment().utc().subtract(1, "days").endOf("day").format(),scale:Scale.DAY});
			if (status === 401) {
				return false;
			} else {
				usage = this.saveDeviceUsages(data, outputunits);
				return usage;
			}
		} else {
			usage = this.saveDeviceUsages(data, outputunits);
	...
	async getEmpDeviceListUsage(outputunits=0) {
		let devices = {};
		...
		if (status === 401) {           // token expired
			...
			if (status === 401) {
				...
				devices = this.saveDeviceUsages(data, outputunits);
				return devices;
			}
		} else {
			devices = this.saveDeviceUsages(data, outputunits);
	...
	saveDeviceUsages(devices, outputunits) {
		const devicesUsage = [];
		devices.forEach(device => {
			devicesUsage.push(device);
			device.channelUsages.forEach(channelUsage => {
				if (outputunits === 1) {
					channelUsage.usageKW = channelUsage.usage * 3600 * 1000 / 1;	// Watt
				} else {
					channelUsage.usageKW = channelUsage.usage * 3600 / 1;			// kW
