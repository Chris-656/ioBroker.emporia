...
class EmVue {
	...
	async login(emporia) {
		...
		try {
			const accessToken = await emporia.getStateAsync("tokens.accessToken");
			const idToken = await emporia.getStateAsync("tokens.accessToken");
			const refreshToken = await emporia.getStateAsync("tokens.accessToken");
			if (accessToken) this.userCred.AccessToken = accessToken.val;
			if (idToken) this.userCred.IdToken = idToken.val;
			if (refreshToken) this.userCred.RefreshToken = refreshToken.val;

			if (!this.userCred.IdToken || !this.userCred.RefreshToken || !this.userCred.AccessToken) {
				const res = await this.getAWSCredentials();
				if (res.AccessToken) this.userCred.AccessToken = res.AccessToken;
				if (res.IdToken) this.userCred.IdToken = res.IdToken;
				if (res.ExpiresIn) this.userCred.ExpiresIn = res.ExpiresIn;

				emporia.setObjectNotExistsAsync(`tokens.accessToken`, { type: "state", common: { name: "firmware", type: "string", role: "name", read: true, write: false }, native: {}, });
				emporia.setState(`tokens.accessToken`, this.userCred.AccessToken, true, true);

				emporia.setObjectNotExistsAsync(`tokens.idToken`, { type: "state", common: { name: "firmware", type: "string", role: "name", read: true, write: false }, native: {}, });
				emporia.setState(`tokens.idToken`, this.userCred.IdToken, true, true);

				emporia.setObjectNotExistsAsync(`tokens.refreshToken`, { type: "state", common: { name: "firmware", type: "string", role: "name", read: true, write: false }, native: {}, });
				emporia.setState(`tokens.refreshToken`, this.userCred.RefreshToken, true, true);
	...
	_storeUserCredentialstoFile(credentials) {
		...
		fs.writeFileSync(`lib/${this.userCred.tokenStorageFile}`, jsonData);
	}

	getUserCredentialsfromFile() {
		const userCred = {};

		const f = fs.readFileSync("lib/keys.json", "utf8");
